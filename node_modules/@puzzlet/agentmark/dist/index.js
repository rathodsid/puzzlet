import default2 from 'node:path';
import default3 from 'node:process';
import { fileURLToPath } from 'node:url';
import yaml from 'js-yaml';
import { z } from 'zod';
import { generateObject, jsonSchema, generateText, streamObject, streamText } from 'ai';
import Ajv from 'ajv';
    ...
    (function(global, factory) {
      ...
      types$12.star.updateContext = function(prevType) {
        if (prevType === types$12._function) {
          ...
          if (this.context[index3] === types2.f_expr) {
            this.context[index3] = types2.f_expr_gen;
          } else {
            this.context[index3] = types2.f_gen;
          }
        }
        this.exprAllowed = true;
      };
...
var Jsep = class _Jsep {
  ...
  runHook(name2, node2) {
    if (_Jsep.hooks[name2]) {
      const env = { context: this, node: node2 };
  ...
  parse() {
    ...
    const node2 = nodes.length === 1 ? nodes[0] : {
      type: _Jsep.COMPOUND,
      body: nodes
    };
    return this.runHook("after-all", node2);
  }
  /**
   ...
   */
  gobbleExpression() {
    const node2 = this.searchHook("gobble-expression") || this.gobbleBinaryExpression();
    this.gobbleSpaces();
    return this.runHook("after-expression", node2);
  }
  /**
  ...
  gobbleBinaryExpression() {
    let node2, biop, prec, stack, biop_info, left, right, i, cur_biop;
   ...
   */
  gobbleToken() {
    let ch, to_check, tc_len, node2;
    this.gobbleSpaces();
    node2 = this.searchHook("gobble-token");
    if (node2) {
      return this.runHook("after-token", node2);
    }
    ch = this.code;
    ...
    if (ch === _Jsep.SQUOTE_CODE || ch === _Jsep.DQUOTE_CODE) {
      node2 = this.gobbleStringLiteral();
    } else if (ch === _Jsep.OBRACK_CODE) {
      node2 = this.gobbleArray();
    } else {
      ...
      if (_Jsep.isIdentifierStart(ch)) {
        node2 = this.gobbleIdentifier();
        if (_Jsep.literals.hasOwnProperty(node2.name)) {
          node2 = {
            type: _Jsep.LITERAL,
            value: _Jsep.literals[node2.name],
            raw: node2.name
          };
        } else if (node2.name === _Jsep.this_str) {
          node2 = { type: _Jsep.THIS_EXP };
        }
      } else if (ch === _Jsep.OPAREN_CODE) {
        node2 = this.gobbleGroup();
      }
    }
    if (!node2) {
      return this.runHook("after-token", false);
    }
    node2 = this.gobbleTokenProperty(node2);
    return this.runHook("after-token", node2);
  }
  /**
   ...
   */
  gobbleIdentifier() {
    let ch = this.code, start2 = this.index;
    if (_Jsep.isIdentifierStart(ch)) {
      this.index++;
    } else {
      this.throwError("Unexpected " + this.char);
  ...
  gobbleArguments(termination) {
    ...
    while (this.index < this.expr.length) {
      ...
      if (ch_i === termination) {
        ...
        const node2 = this.gobbleExpression();
        if (!node2 || node2.type === _Jsep.COMPOUND) {
          this.throwError("Expected comma");
        }
        args.push(node2);
      }
    ...
    function gobbleObjectExpression(env) {
      if (this.code === OCURLY_CODE) {
        ...
        while (!isNaN(this.code)) {
          ...
          this.gobbleSpaces();
          if (key.type === jsep2.IDENTIFIER && (this.code === jsep2.COMMA_CODE || this.code === CCURLY_CODE)) {
            properties.push({
              type: PROPERTY,
              computed: false,
              key,
            ...
            this.index++;
            const value2 = this.gobbleExpression();
            if (!value2) {
              this.throwError("unexpected object property");
            }
            const computed = key.type === jsep2.ARRAY_EXP;
            properties.push({
              type: PROPERTY,
              computed,
...
var VFile = class {
  ...
  constructor(value2) {
    ...
    this.cwd = "cwd" in options2 ? "" : default3.cwd();
    this.data = {};
    this.history = [];
    this.messages = [];
    this.value;
...
var Processor = class _Processor extends CallableInstance {
  ...
  use(value2, ...parameters) {
    ...
    if (value2 === null || value2 === void 0) ; else if (typeof value2 === "function") {
...
function resolveToLabelEnd(events, context) {
  ...
  media = push(media, resolveAll(context.parser.constructs.insideSpan.null, events.slice(open + offset2 + 4, close2 - 3), context));
...
function fromMarkdown(value2, encoding, options2) {
  ...
  return compiler(options2)(postprocess(parse(options2).document().write(preprocess()(value2, encoding, true))));
}
function compiler(options2) {
  ...
  function compile(events) {
    ...
    while (++index3 < events.length) {
      ...
      if (own3.call(handler2, events[index3][1].type)) {
  ...
  function onexitcharacterreferencevalue(token) {
    ...
    let value2;
    if (type) {
      value2 = decodeNumericCharacterReference(data3, type === "characterReferenceMarkerNumeric" ? 10 : 16);
...
function parseEntities(value2, options2 = {}) {
  ...
  if (options2.position) {
    if ("start" in options2.position || "indent" in options2.position) {
      ...
      point3 = options2.position.start;
    } else {
      point3 = options2.position;
    }
  }
  ...
  while (++index3 <= value2.length) {
    ...
    if (character === 38) {
      ...
      let reference = "";
      if (!terminated && options2.nonTerminated === false) ; else if (!characters) {
        if (type !== "named") {
          warning(4, diff);
        }
      } else if (type === "named") {
    ...
    if (node2.attributes && node2.attributes.length > 0) {
      ...
      while (++index3 < node2.attributes.length) {
        ...
        if (attribute.type === "mdxJsxExpressionAttribute") {
          ...
          let right = "";
          if (value3 === null || value3 === void 0) ; else if (typeof value3 === "object") {
            right = "{" + (value3.value || "") + "}";
          } else {
...
function visit(tree, visitor) {
  ...
  function build(node2, key, index3, parents) {
    if (nodelike(node2)) {
      visit3.displayName = "node (" + color(node2.type) + ")";
    }
    return visit3;
    function visit3() {
      const result = enter ? toResult(enter(node2, key, index3, parents)) : [];
      if (result[0] === EXIT) {
        return result;
      }
      if (result[0] !== SKIP) {
      ...
      return leave ? toResult(leave(node2, key, index3, parents)) : result;
    }
  }
}
...
function eventsToAcorn(events, options2) {
  ...
  const isEmptyExpression = options2.expression && empty2(source);
  ...
  try {
    estree = options2.expression && !isEmptyExpression ? options2.acorn.parseExpressionAt(value2, 0, acornConfig) : options2.acorn.parse(value2, acornConfig);
  } catch (error_) {
    const error = (
      /** @type {AcornError} */
      error_
  ...
  if (estree && options2.expression && !isEmptyExpression) {
    if (empty2(value2.slice(estree.end, value2.length - suffix.length))) {
      estree = {
        type: "Program",
        start: 0,
...
function empty2(value2) {
  return /^\s*$/.test(value2.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\r\n]*(\r\n|\n|\r)/g, ""));
}
  ...
  while (++index3 < events.length) {
    ...
    if (event[0] === "enter") {
      ...
      if (type === "lineEnding" || tokenTypes.includes(type)) {
        ...
        const value2 = serializeChunks2(chunks);
...
function factoryMdxExpression(effects, ok3, type, markerType, chunkType, acorn, acornOptions, addResult, spread, allowEmpty, allowLazy) {
  ...
  function before(code2) {
    ...
    if (code2 === 125 && size === 0) {
      const next = acorn ? mdxExpressionParse.call(self2, acorn, acornOptions, chunkType, eventStart, pointStart, allowEmpty || false, spread || false) : {
        type: "ok",
        estree: void 0
      };
      if (next.type === "ok") {
...
function jsxText(acorn, options2) {
  ...
  function tokenizeJsxText(effects, ok3, nok) {
    return factoryTag.call(this, effects, ok3, nok, acorn, options2.acornOptions, options2.addResult, true, "mdxJsxTextTag", "mdxJsxTextTagMarker", "mdxJsxTextTagClosingMarker", "mdxJsxTextTagSelfClosingMarker", "mdxJsxTextTagName", "mdxJsxTextTagNamePrimary", "mdxJsxTextTagNameMemberMarker", "mdxJsxTextTagNameMember", "mdxJsxTextTagNamePrefixMarker", "mdxJsxTextTagNameLocal", "mdxJsxTextTagExpressionAttribute", "mdxJsxTextTagExpressionAttributeMarker", "mdxJsxTextTagExpressionAttributeValue", "mdxJsxTextTagAttribute", "mdxJsxTextTagAttributeName", "mdxJsxTextTagAttributeNamePrimary", "mdxJsxTextTagAttributeNamePrefixMarker", "mdxJsxTextTagAttributeNameLocal", "mdxJsxTextTagAttributeInitializerMarker", "mdxJsxTextTagAttributeValueLiteral", "mdxJsxTextTagAttributeValueLiteralMarker", "mdxJsxTextTagAttributeValueLiteralValue", "mdxJsxTextTagAttributeValueExpression", "mdxJsxTextTagAttributeValueExpressionMarker", "mdxJsxTextTagAttributeValueExpressionValue");
  }
}
...
function jsxFlow(acorn, options2) {
  ...
  function tokenizeJsxFlow(effects, ok3, nok) {
    ...
    function before(code2) {
      return factoryTag.call(self2, effects, after, nok, acorn, options2.acornOptions, options2.addResult, false, "mdxJsxFlowTag", "mdxJsxFlowTagMarker", "mdxJsxFlowTagClosingMarker", "mdxJsxFlowTagSelfClosingMarker", "mdxJsxFlowTagName", "mdxJsxFlowTagNamePrimary", "mdxJsxFlowTagNameMemberMarker", "mdxJsxFlowTagNameMember", "mdxJsxFlowTagNamePrefixMarker", "mdxJsxFlowTagNameLocal", "mdxJsxFlowTagExpressionAttribute", "mdxJsxFlowTagExpressionAttributeMarker", "mdxJsxFlowTagExpressionAttributeValue", "mdxJsxFlowTagAttribute", "mdxJsxFlowTagAttributeName", "mdxJsxFlowTagAttributeNamePrimary", "mdxJsxFlowTagAttributeNamePrefixMarker", "mdxJsxFlowTagAttributeNameLocal", "mdxJsxFlowTagAttributeInitializerMarker", "mdxJsxFlowTagAttributeValueLiteral", "mdxJsxFlowTagAttributeValueLiteralMarker", "mdxJsxFlowTagAttributeValueLiteralValue", "mdxJsxFlowTagAttributeValueExpression", "mdxJsxFlowTagAttributeValueExpressionMarker", "mdxJsxFlowTagAttributeValueExpressionValue")(code2);
    }
    function after(code2) {
      return markdownSpace(code2) ? factorySpace(effects, end, "whitespace")(code2) : end(code2);
    }
...
function mdxjsEsm(options2) {
  ...
  function tokenizeExportImport(effects, ok3, nok) {
    ...
    function atEnd(code2) {
      ...
      Object.assign(effects.exit("mdxjsEsm"), options2.addResult ? {
...
function configure2(base, extension2) {
  ...
  if (extension2.extensions) {
    while (++index3 < extension2.extensions.length) {
      configure2(base, extension2.extensions[index3]);
    }
  }
  for (key in extension2) {
...
function toResult2(value2) {
  ...
  return value2 === null || value2 === void 0 ? empty3 : [value2];
}
...
// ../../node_modules/@puzzlet/templatedx/dist/index.js
...
function isMdxJsxElement(node2) {
  return isMdxJsxFlowElement(node2) || isMdxJsxTextElement(node2);
}
function isMdxJsxFlowElement(node2) {
  return node2.type === NODE_TYPES.MDX_JSX_FLOW_ELEMENT;
}
function isMdxJsxTextElement(node2) {
  return node2.type === NODE_TYPES.MDX_JSX_TEXT_ELEMENT;
}
function isParentNode(node2) {
  return "children" in node2 && Array.isArray(node2.children);
}
var getFrontMatter = (tree) => {
  const frontmatterNode = tree.children.find(
    (node2) => node2.type === NODE_TYPES.YAML
  );
  return yaml.load((frontmatterNode == null ? void 0 : frontmatterNode.value) || "");
};
function parse4(mdxContent) {
...
function getFunctionBody(childNode) {
  ...
  const functionBodyTree = parse4(functionBodyCode);
...
function unwrapFragments(nodes) {
  ...
  for (const node2 of nodes) {
    if (isFragmentNode(node2)) {
      if (node2.children) {
        const childNodes = unwrapFragments(node2.children);
        unwrappedNodes.push(...childNodes);
      }
    } else {
      unwrappedNodes.push(node2);
    }
  }
  return unwrappedNodes;
}
function isFragmentNode(node2) {
  return node2.type === NODE_TYPES.MDX_JSX_FLOW_ELEMENT && node2.name === null;
}
...
var FilterRegistry = class {
  static register(name2, filterFunction) {
    this.filters.set(name2, filterFunction);
  }
  static get(name2) {
    return this.filters.get(name2);
  }
  static getAll() {
    return new Map(this.filters);
  }
  static remove(name2) {
    this.filters.delete(name2);
  }
  static removeAll() {
    this.filters.clear();
  }
...
var Scope3 = class _Scope {
  ...
  setShared(key, value2) {
    this.shared[key] = value2;
  }
  setLocal(key, value2) {
    this.variables[key] = value2;
  }
  createChild(variables = {}) {
    return new _Scope(variables, this.shared, this);
  }
};
function mdxExpressionToMarkdown2() {
  return {
    handlers: {
...
function handleMdxExpression2(node2, parent, state) {
  const value2 = node2.value || "";
  const result = state.indentLines(value2, function(line, index3, blank) {
    return (index3 === 0 || blank ? "" : "  ") + line;
  });
...
function ccount2(value2, character) {
  const source = String(value2);
  if (typeof character !== "string") {
    throw new TypeError("Expected character");
  }
...
var defaultSubsetRegex2 = /["&'<>`]/g;
var surrogatePairsRegex2 = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g;
var controlCharactersRegex2 = (
  // eslint-disable-next-line no-control-regex, unicorn/no-hex-escape
  /[\x01-\t\v\f\x0E-\x1F\x7F\x81\x8D\x8F\x90\x9D\xA0-\uFFFF]/g
);
var regexEscapeRegex2 = /[|\\{}()[\]^$+*?.]/g;
var subsetToRegexCache2 = /* @__PURE__ */ new WeakMap();
function core2(value2, options2) {
  value2 = value2.replace(
    options2.subset ? charactersToExpressionCached2(options2.subset) : defaultSubsetRegex2,
    basic
  );
  if (options2.subset || options2.escapeOnly) {
    return value2;
  }
  return value2.replace(surrogatePairsRegex2, surrogate).replace(controlCharactersRegex2, basic);
  function surrogate(pair, index3, all2) {
    return options2.format(
      (pair.charCodeAt(0) - 55296) * 1024 + pair.charCodeAt(1) - 56320 + 65536,
      all2.charCodeAt(index3 + 2),
...
function charactersToExpressionCached2(subset) {
  let cached = subsetToRegexCache2.get(subset);
  if (!cached) {
    cached = charactersToExpression2(subset);
    subsetToRegexCache2.set(subset, cached);
  }
  return cached;
}
function charactersToExpression2(subset) {
  ...
  let index3 = -1;
  while (++index3 < subset.length) {
    groups.push(subset[index3].replace(regexEscapeRegex2, "\\$&"));
  }
  return new RegExp("(?:" + groups.join("|") + ")", "g");
}
...
function stringifyEntitiesLight2(value2, options2) {
  return core2(value2, Object.assign({ format: formatBasic2 }, options2));
}
var indent2 = "  ";
function mdxJsxToMarkdown2(options2) {
  const options_ = {};
  const quote = options_.quote || '"';
  const quoteSmart = options_.quoteSmart || false;
  const tightSelfClosing = options_.tightSelfClosing || false;
  ...
  function mdxElement(node2, _, state, info) {
    const flow3 = node2.type === "mdxJsxFlowElement";
    const selfClosing = node2.name ? !node2.children || node2.children.length === 0 : false;
    const depth = inferDepth2(state);
    const currentIndent = createIndent2(depth);
    const trackerOneLine = state.createTracker(info);
    const trackerMultiLine = state.createTracker(info);
    const serializedAttributes = [];
    const prefix = (flow3 ? currentIndent : "") + "<" + (node2.name || "");
    ...
    if (node2.attributes && node2.attributes.length > 0) {
      ...
      while (++index3 < node2.attributes.length) {
        ...
        if (attribute.type === "mdxJsxExpressionAttribute") {
          ...
          const value22 = attribute.value;
          const left = attribute.name;
          let right = "";
          if (value22 === null || value22 === void 0) ; else if (typeof value22 === "object") {
            right = "{" + (value22.value || "") + "}";
          } else {
            const appliedQuote = quoteSmart && ccount2(value22, quote) > ccount2(value22, alternative) ? alternative : quote;
            right = appliedQuote + stringifyEntitiesLight2(value22, { subset: [appliedQuote] }) + appliedQuote;
          }
          result = left + (right ? "=" : "") + right;
        }
        serializedAttributes.push(result);
    ...
    if (attributesOnTheirOwnLine) {
      ...
      let index3 = -1;
      while (++index3 < serializedAttributes.length) {
        serializedAttributes[index3] = currentIndent + indent2 + serializedAttributes[index3];
      }
      value2 += tracker.move(
        "\n" + serializedAttributes.join("\n") + "\n" + currentIndent
      );
    ...
    if (node2.children && node2.children.length > 0) {
      if (node2.type === "mdxJsxTextElement") {
        ...
        tracker.shift(2);
        value2 += tracker.move("\n");
        value2 += tracker.move(containerFlow3(node2, state, tracker.current()));
        value2 += tracker.move("\n");
      }
    }
    if (!selfClosing) {
...
function containerFlow3(parent, state, info) {
  const indexStack = state.indexStack;
  const children = parent.children;
  const tracker = state.createTracker(info);
  const currentIndent = createIndent2(inferDepth2(state));
  const results = [];
  let index3 = -1;
  indexStack.push(-1);
  while (++index3 < children.length) {
...
function createIndent2(depth) {
  return indent2.repeat(depth);
}
function peekElement2() {
  return "<";
}
function mdxjsEsmToMarkdown2() {
  return { handlers: { mdxjsEsm: handleMdxjsEsm2 } };
}
function handleMdxjsEsm2(node2) {
  return node2.value || "";
}
function mdxToMarkdown2(options2) {
  return {
    extensions: [
      mdxExpressionToMarkdown2(),
      mdxJsxToMarkdown2(),
      mdxjsEsmToMarkdown2()
    ]
  };
...
var options = {
  extensions: [mdxToMarkdown2()]
};
var toMdxMarkdown = (node2) => {
  return toMarkdown(node2, options);
};
var nodeHelpers = {
  isMdxJsxElement,
  isMdxJsxFlowElement,
...
var NodeTransformer = class _NodeTransformer {
  ...
  async transformNode(node2) {
    if (node2.type === NODE_TYPES.MDX_TEXT_EXPRESSION || node2.type === NODE_TYPES.MDX_FLOW_EXPRESSION) {
      return this.evaluateExpressionNode(node2);
    }
    if (isMdxJsxElement(node2)) {
      return await this.processMdxJsxElement(node2);
    }
    if (this.isFragmentNode(node2)) {
      const processedChildren = await Promise.all(
        node2.children.map(async (child) => {
  ...
  isFragmentNode(node2) {
    return isMdxJsxElement(node2) && (node2.name === null || node2.name === "" || node2.name === "Fragment" || node2.name === "React.Fragment");
  }
  evaluateExpressionNode(node2) {
    const expression = node2.value;
    try {
  ...
  evaluateJsepExpression(node2) {
    switch (node2.type) {
      case "BinaryExpression":
        return this.evaluateBinaryExpression(node2);
      case "UnaryExpression":
        return this.evaluateUnaryExpression(node2);
      case "Literal":
        return node2.value;
      case "Identifier":
        return this.resolveVariable(node2.name);
      case "MemberExpression":
        return this.evaluateMemberExpression(node2);
      case "CallExpression":
        return this.evaluateCallExpression(node2);
      case "ArrayExpression":
        return this.evaluateArrayExpression(node2);
      case "ObjectExpression":
        return this.evaluateObjectExpression(node2);
      default:
        throw new Error(`Unsupported node type: ${node2.type}`);
    }
  }
  evaluateArrayExpression(node2) {
    return node2.elements.map((element) => this.evaluateJsepExpression(element));
  }
  evaluateObjectExpression(node2) {
    const obj = {};
    for (const property of node2.properties) {
      ...
      const value2 = this.evaluateJsepExpression(property.value);
      obj[key] = value2;
    }
    return obj;
  }
  evaluateCallExpression(node2) {
    ...
    const args = node2.arguments.map((arg) => this.evaluateJsepExpression(arg));
  ...
  evaluateBinaryExpression(node2) {
    ...
    const left = this.evaluateJsepExpression(node2.left);
    return operation(left, node2.right);
  }
  evaluateUnaryExpression(node2) {
    const argument = this.evaluateJsepExpression(node2.argument);
    switch (node2.operator) {
      ...
      case "!":
        return !argument;
      default:
        throw new Error(`Unsupported operator: ${node2.operator}`);
    }
  }
  evaluateMemberExpression(node2) {
    const object = this.evaluateJsepExpression(node2.object);
    const property = node2.computed ? this.evaluateJsepExpression(node2.property) : node2.property.name;
  ...
  async processMdxJsxElement(node2) {
    try {
      const tagName = node2.name;
      const plugin = TagPluginRegistry.get(tagName);
      if (plugin) {
        const props = this.evaluateProps(node2);
        const pluginContext = {
        ...
        const result = await plugin.transform(props, node2.children, pluginContext);
        return result;
      } else {
        const newNode = { ...node2 };
        const processedChildren = await Promise.all(
  ...
  evaluateProps(node2) {
    ...
    for (const attr of node2.attributes) {
      if (attr.type === MDX_JSX_ATTRIBUTE_TYPES.MDX_JSX_ATTRIBUTE) {
        ...
        throw new Error(
          `Unsupported attribute type in component <${node2.name}>.`
        );
      }
    }
    return props;
...
var transformTree = async (tree, props = {}, shared = {}) => {
  const scope = new Scope3({ props }, shared);
...
async function processMdxContent(content3, absolutePath, callStack, processedFiles, contentLoader) {
  ...
  tree.children = tree.children.filter(
    (node2) => node2.type !== NODE_TYPES.MDX_JSX_ESM
  );
...
function removeComments(tree) {
  visit2(tree, (node2, index3, parent) => {
    if (isCommentNode(node2) && parent) {
...
function isCommentNode(node2) {
  if (node2.type === NODE_TYPES.MDX_FLOW_EXPRESSION || node2.type === NODE_TYPES.MDX_TEXT_EXPRESSION) {
    const value2 = node2.value.trim();
    return value2.startsWith("/*") && value2.endsWith("*/") || value2.startsWith("//");
  }
  return false;
}
function extractImports(tree, absolutePath) {
  const imports = {};
  visit2(tree, NODE_TYPES.MDX_JSX_ESM, (node2) => {
    var _a;
    const estree = (_a = node2.data) == null ? void 0 : _a.estree;
    if (!estree) {
    ...
    for (const stmt of estree.body) {
      if (stmt.type === "ImportDeclaration") {
        ...
        if (stmt.specifiers.some(
          ...
          throw new Error(
            `Only default imports are supported. Invalid import in ${absolutePath}: ${node2.value.trim()}`
          );
        }
        if (defaultSpecifier) {
          ...
          throw new Error(
            `Invalid import in ${absolutePath}: ${node2.value.trim()}`
          );
        }
      } else if (stmt.type.startsWith("Export")) {
        throw new Error(
          `Exports are not supported. Found in ${absolutePath}: ${node2.value.trim()}`
        );
      }
    }
  });
...
function inlineJsxElements(tree, componentASTs, parentProps = {}) {
  ...
  visit2(
    ...
    (node2, index3, parent) => {
      const componentName = node2.name;
      if (componentASTs[componentName]) {
        const componentNodes = cloneObject(componentASTs[componentName]);
        const props = extractRawProps(node2, parentProps);
        const childrenContent = node2.children || [];
        ...
        parent.children.splice(index3, 1, ...processedComponentNodes.flat());
        replaced = true;
      }
    }
  );
...
function extractRawProps(node2, parentProps) {
  const props = {};
  for (const attr of node2.attributes) {
    if (attr.type === MDX_JSX_ATTRIBUTE_TYPES.MDX_JSX_ATTRIBUTE) {
      if (attr.value === null || typeof attr.value === "string") {
      ...
      throw new Error(
        `Only literal attribute values are supported. Invalid attribute in component <${node2.name}>.`
      );
    }
  }
  return props;
...
function inlineComponentsAndResolveProps(node2, props, childrenContent, componentASTs) {
  if (node2.type === NODE_TYPES.MDX_TEXT_EXPRESSION || node2.type === NODE_TYPES.MDX_FLOW_EXPRESSION) {
    if (node2.value === "props.children") {
    ...
    } else if (node2.value.includes("props.")) {
  ...
  if (isMdxJsxElement(node2)) {
    ...
    if (componentASTs[componentName]) {
      const componentNodes = cloneObject(componentASTs[componentName]);
      const newProps = extractRawProps(node2, props);
      const childrenContent2 = node2.children || [];
      const processedComponentNodes = componentNodes.map(
        (childNode) => inlineComponentsAndResolveProps(
          childNode,
          newProps,
...
function combinedNodesIntoParagraph(nodes) {
  ...
  nodes.forEach((node2, index3) => {
    if (node2.type === NODE_TYPES.PARAGRAPH || node2.type === NODE_TYPES.LIST) {
      contentChildren.push(...node2.children);
    } else {
      contentChildren.push(node2);
    }
    if (index3 !== nodes.length - 1) {
...
var replace = (input, search2, replace2) => {
  if (typeof input !== "string") return input;
  return input.split(search2).join(replace2);
};
var urlencode = (input) => {
  if (typeof input !== "string") return input;
...
FilterRegistry.register("join", join2);
...
async function load(path2) {
  const file = await readFile(path2);
  const componentLoader = async (path22) => readFile(path22);
  return bundle(file, getDirname(path2), componentLoader);
}

// src/model-plugin-registry.ts
var ModelPluginRegistry = class {
  static register(modelPlugin, ids) {
    for (const id of ids) {
...
ModelPluginRegistry.plugins = /* @__PURE__ */ new Map();

// src/extract-text.ts
var ExtractTextPlugin = class extends TagPlugin {
  async transform(_props, children, pluginContext) {
    const { scope, tagName, createNodeTransformer, nodeHelpers: nodeHelpers2 } = pluginContext;
    if (!tagName) {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        const extractedText = nodeHelpers2.toMarkdown({
          type: "root",
          // @ts-ignore
...
var AgentMarkBaseSettingsSchema = z.object({
...
AgentMarkBaseSettingsSchema.extend({
  tools: z.record(
    z.object({
...
var AgentMarkSettingsSchema = AgentMarkBaseSettingsSchema.extend({
...
}).refine((data2) => "schema" in data2 ? !("tools" in data2) : true, {
...
var AgentMarkSchema = z.object({
  name: z.string(),
  messages: z.array(ChatMessageSchema),
  metadata: MetadataSchema
});

// src/tool-plugin-registry.ts
var ToolPluginRegistry = class {
  static register(toolFunction, name2) {
    if (this.tools.has(name2)) {
      throw new Error(`Tool with the name "${name2}" is already registered.`);
    }
    this.tools.set(name2, toolFunction);
  }
  ...
  static getTool(name2) {
    return this.tools.get(name2);
  }
  static removeTool(name2) {
    if (!this.tools.delete(name2)) {
      throw new Error(`Tool with the name "${name2}" does not exist.`);
    }
  }
  static clearRegistry() {
    this.tools.clear();
...
ToolPluginRegistry.tools = /* @__PURE__ */ new Map();

// src/utils.ts
var OUTPUT_VERSION = "v2.0";
function toFrontMatter(content3) {
  function jsonToFrontMatter(json, indent3 = 0) {
    let frontMatter = "";
...
`;
          frontMatter += jsonToFrontMatter(value2, indent3 + 1);
...
`;
              frontMatter += jsonToFrontMatter(item, indent3 + 2);
            } else {
...
`;
          ...
          frontMatter += `${indentation}${key}: ${value2}
`;
        }
      }
    }
...
function createToolsConfig(tools) {
  if (!tools) return void 0;
  return Object.entries(tools).reduce((acc, [toolName, toolData]) => {
...
function getBaseSettings(config, model, messages2) {
  return {
    messages: messages2,
    model,
...
async function runInference(config, model, messages2, options2) {
  const baseConfig = getBaseSettings(config, model, messages2);
  baseConfig.experimental_telemetry = options2 == null ? void 0 : options2.telemetry;
...
async function streamInference(config, model, messages2, options2) {
  const baseConfig = getBaseSettings(config, model, messages2);
  baseConfig.experimental_telemetry = options2 == null ? void 0 : options2.telemetry;
  const settings = AgentMarkSettingsSchema.parse(config);
  if ("schema" in settings) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        resolve({
          resultStream: partialObjectStream,
          version: OUTPUT_VERSION,
          usage,
...
function getMessages(extractedFields) {
  const messages2 = [];
  extractedFields.forEach((field, index3) => {
    const fieldName = field.name.toLocaleLowerCase();
    if (index3 !== 0 && fieldName === "system") {
      throw new Error(`System message may only be the first message only: ${field.content}`);
    }
    messages2.push({ role: fieldName, content: field.content });
  });
  return messages2;
}
async function getRawConfig(ast, props) {
  var _a, _b;
  ...
  const messages2 = getMessages(await Promise.all(extractedFieldPromises));
  frontMatter.metadata.model.settings = ((_b = (_a = frontMatter.metadata) == null ? void 0 : _a.model) == null ? void 0 : _b.settings) || {};
  const agentMark = AgentMarkSchema.parse({
...
async function runInference2(ast, props, options2) {
  var _a, _b, _c, _d;
  ...
  const inferenceOptions = {
    ...options2,
    telemetry: {
      ...options2 == null ? void 0 : options2.telemetry,
      metadata: {
        ...(_a = options2 == null ? void 0 : options2.telemetry) == null ? void 0 : _a.metadata,
  ...
  if ((_d = (_c = (_b = frontMatter.metadata) == null ? void 0 : _b.model) == null ? void 0 : _c.settings) == null ? void 0 : _d.schema) {
...
async function streamInference2(ast, props, options2) {
  var _a;
  const agentMark = await getRawConfig(ast, props);
  const plugin = ModelPluginRegistry.getPlugin(
    agentMark.metadata.model.name
...
function serialize(completionParams, model, promptName) {
  const plugin = ModelPluginRegistry.getPlugin(model);
  return plugin == null ? void 0 : plugin.serialize(completionParams, promptName, PluginAPI);
}
async function deserialize(ast, props = {}, config) {
  const agentMark = await getRawConfig(ast, props);
  const plugin = ModelPluginRegistry.getPlugin(
    agentMark.metadata.model.name
  );
...
function createTemplateRunner(ast) {
  return {
    run: (props, options2) => runInference2(ast, props, options2),
    stream: (props, options2) => streamInference2(ast, props, options2),
    compile: (props) => getRawConfig(ast, props),
    deserialize: (props, config) => deserialize(ast, props, config)
  };
}
var FileLoader = class {
  constructor(rootDir, createRunner) {
...
export { FileLoader, FilterRegistry, ModelPluginRegistry, PluginAPI, ToolPluginRegistry, createTemplateRunner, deserialize, getFrontMatter, getModel, getRawConfig, load, bundle as parse, runInference2 as runInference, serialize, streamInference2 as streamInference, toFrontMatter };
